#!/usr/bin/env python3
"""User-editable settings persisted under ``<DATA_DIR>/user_settings.json``.
...
JSON inside the project ``DATA_DIR`` (see ``app.storage.DATA_DIR``) so they sit
next to the app's other runtime data and stay out of git.

edb_make 원본은 ``.app_runtime/user_settings.json`` 에 저장했지만, hwp_make로
포팅하면서 저장 위치만 ``<DATA_DIR>/user_settings.json`` 으로 바꿨다. 나머지
키 저장 / ``apply_to_env`` / 마스킹된 요약 패턴은 그대로 유지한다 (원시 키는
절대 그대로 echo 하지 않는다).
"""
...
def _resolve_runtime_dir(runtime_dir: Path | None) -> Path:
    ...
    A caller may still pass an explicit ``runtime_dir`` (edb API compatibility);
    when omitted (the hwp_make default), settings live directly under the
    project ``DATA_DIR``. Imported lazily to avoid an import cycle with
    ``app.storage``.
    """
...
def settings_path(runtime_dir: Path | None = None) -> Path:
    return _resolve_runtime_dir(runtime_dir) / _SETTINGS_FILENAME


def load_user_settings(runtime_dir: Path | None = None) -> dict[str, Any]:
...
def save_user_settings(runtime_dir: Path | None, settings: dict[str, Any]) -> Path:
    resolved = _resolve_runtime_dir(runtime_dir)
    resolved.mkdir(parents=True, exist_ok=True)
    path = settings_path(resolved)
...
def update_gemini_api_key(runtime_dir: Path | None, raw_key: str | None) -> dict[str, Any]:
...
def update_openai_api_key(runtime_dir: Path | None, raw_key: str | None) -> dict[str, Any]:
