...
def write_hwpx(
    ...
    column_body_height = (
        ...
        else KICE_MATH_COLUMN_BODY_HEIGHT
    )
    ...
    def add_single_para(
        ...
        **attrs: str,
        ...
        if pending_page_break:
            ...
            columns > 1
            and allow_column_break
            and not source_layout_flow
            and flow_y > 0
            and flow_y + para_height > column_body_height
            and (not source_layout_flow or current_flow_column < columns)
        ):
            attrs.setdefault("columnBreak", "1")
            flow_y = 0
            current_flow_column = min(columns, current_flow_column + 1)
    ...
    def add_wrapped_line(
        ...
        **attrs: str,
        ...
        for wrapped in wrap_line_parts(line, limits.get(style, 68)):
            add_single_para(
                wrapped,
                style,
                center=center,
    ...
    for index, problem in enumerate(problems, start=1):
        if source_layout_flow and previous_problem is not None:
            ...
            if current_page > 0 and previous_page > 0 and current_page != previous_page:
                ...
                if target_column <= 0 or current_flow_column < target_column:
                    pending_column_break = True
        elif native_math and template.key == "kice_math" and columns > 1:
