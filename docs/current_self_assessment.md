...
> 최신 국어·수학·영어 재평가는 [본문 대조·기준선 수정과 5분야 평가](audits/2026-09-22/body-scope-baseline-progress.md)를 따릅니다. 36쪽 엄격 배치 평균은 **86.47 → 87.31**, 전체 회귀는 **101 통과·11 생략·1 실패**, 정상 패키지 33개 유효입니다. 머리말·꼬리말이 본문 대조에 섞이던 오류를 수정해 국어 원문 텍스트층 27,028자가 모두 대응합니다. 본문을 머리말/꼬리말로 옮긴 변조는 두 API 모드 모두 422입니다. 수학 22개 단 시작점의 세로 기준선은 0.1px를 통과하고, 영어 본문 10줄의 한 문단·790자 추가 편집도 유지됩니다. **93점 미달입니다.** 6쪽 배치 점수 하락, 그래프 표기 중복, 일부 가로·세로 좌표 오차, 원본 글꼴과 한컴 GUI 미검증이 남았습니다. 텍스트 대응이나 배치 평균을 전체 의미 정확도·5분야 종합 점수로 사용하지 않습니다.
