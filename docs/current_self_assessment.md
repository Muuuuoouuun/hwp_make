...
| 통합 자동 관문 | 깨끗한 재실행 19 PASS, 1 SKIP, 0 FAIL (121.8초) | 그린, 인자 필수 validator 1개는 자동화 필요 |
...
| PDF 원본 레이아웃 HWPX | 부분 완료 | flow writer, 2단, 구분선, 셀 여백, 편집 텍스트, 지역 이미지가 연결되어 있고 overflow/단 침범이 없다. | 실물 원본과 글자 크기·여백·표/셀 밀도를 시각 비교해 named profile로 고정한다. |
...
| 자동 회귀와 재현성 | 부분 완료 | 18개 통합 대상 중 17 PASS, 1 SKIP, 0 FAIL이며 실물/합성 렌더 리포트와 phase timing이 있다. 전체 실행 시간은 218.8초였다. | 인자 필수 validator의 SKIP을 자동 fixture로 바꾸고, 각 검사 데이터 디렉터리를 격리하며, 실패 시 stderr와 artifact를 보존한다. |
