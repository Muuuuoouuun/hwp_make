...
| 통합 자동 관문 | 깨끗한 재실행 20 PASS, 1 SKIP, 0 FAIL (141.7초) | 그린, 인자 필수 validator 1개는 자동화 필요 |
...
| PDF 원본 레이아웃 HWPX | 부분 완료 | flow writer, 2단, 구분선, 셀 여백, 편집 텍스트, 지역 이미지가 연결되어 있다. 전과목 52쪽을 20.494초에 변환하고 한국사 3,682개 벡터 드로잉 페이지의 자료 박스를 보존한다. | 실물 원본과 글자 크기·여백·표/셀 밀도를 전 페이지 시각 비교해 named profile로 고정한다. |
| 타이포그래피 | 부분 완료 | 신명조 계열, Times New Roman, 돋움/중고딕, 장평·자간·165% 줄간격 XML 검증이 통과한다. | 실제 한글 렌더에서 대체 폰트 여부와 과목별 profile을 비교한다. |
| 편집 가능성과 이미지 fallback | 부분 완료 | full-page raster를 성공으로 보지 않고 181/184 문항에 편집 텍스트가 있다. | 진짜 그림과 복원 실패 fallback을 리포트에서 분리하고 사용자에게 문항별 상태를 보여 준다. |
...
| 웹 UI 작업 흐름 | 미검증 | CRUD, import, 원본 레이아웃 export, preview, export API 호출은 연결되어 있고 수식 payload JS 검증이 있다. | 실제 브라우저에서 업로드→편집→미리보기→다운로드 E2E, 오류 상태, 접근성, 큰 파일 진행 상태를 검증한다. |
...
| 자동 회귀와 재현성 | 부분 완료 | 21개 통합 대상 중 20 PASS, 1 SKIP, 0 FAIL이며 실물/합성 렌더 리포트, 과목별 벤치마크, 10초 밀집 벡터 성능 게이트가 있다. 전체 실행 시간은 141.7초였다. | 인자 필수 validator의 SKIP을 자동 fixture로 바꾸고, 각 검사 데이터 디렉터리를 격리하며, 실패 시 stderr와 artifact를 보존한다. |
...
5. **전 페이지 시각 기준 고정**: flow HWPX 전체 페이지 contact sheet와 원본 비교를 자동화한다.
