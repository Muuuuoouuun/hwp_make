...
`/api/pdf-layout-export` 경로입니다. PDF 한 부를 문항 DB에 넣기보다, 원본 시험지 배치를 유지하는 좌표 기반 편집형 HWPX로 직접 복원합니다.
...
- `quality.objective_score`는 목표 95점입니다. 산식은 레이아웃 싱크, 폰트/장평/자간/줄간격 profile, 네이티브 수식 커버리지, 전체 균형, 페이징, 편집 가능 텍스트, editor-open safety 검증을 합산합니다.
...
6. `/api/pdf-layout-export` 산출물은 `objective_score` 95점 이상과 `editor-open safety verification passed`를 동시에 만족해야 합니다.
