...
def main() -> int:
    ...
    # 홀드아웃 실물 PDF는 저작권상 로컬 전용(data/ 는 gitignore) — 없으면 저장소 관례대로 SKIP(exit 2).
    missing_sources = [
        str(source_root / group / f"{subject}.pdf")
        for group, subject in CASES
        if not (source_root / group / f"{subject}.pdf").is_file()
    ]
    if missing_sources:
        print("SKIP: unseen exam source PDFs missing (local-only samples):")
